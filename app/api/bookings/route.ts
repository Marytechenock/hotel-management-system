...
export async function POST(req: Request) {
  ...
  try {
    const created = await prisma.$transaction(async (tx) => {
      ...
      const hasOverlap = existing.some((b) => overlaps(parsed.data.checkIn, parsed.data.checkOut, b.checkIn, b.checkOut))
      ...
      if (parsed.data.status === 'checked_in') {
        await tx.room.update({ where: { id: parsed.data.roomId }, data: { status: 'occupied' } })
      } else if (parsed.data.status === 'confirmed') {
        await tx.room.update({ where: { id: parsed.data.roomId }, data: { status: 'reserved' } })
      }

      return createdBooking
    })

    return NextResponse.json({ ...created, source: parsed.data.source }, { status: 201 })
  } catch (e: any) {
    const msg = typeof e?.message === 'string' ? e.message : ''
    if (msg === 'ROOM_NOT_FOUND') return NextResponse.json({ error: 'Room not found' }, { status: 400 })
    if (msg === 'ROOM_PROPERTY_MISMATCH') return NextResponse.json({ error: 'Room does not belong to property' }, { status: 400 })
    if (msg === 'ROOM_ALREADY_BOOKED') return NextResponse.json({ error: 'Room is already booked for those dates' }, { status: 409 })
